...
def binary_algorithm(grid: Grid):
    ...
    for i in range(grid.shape[0] - 1, -1, -1):
        for j in range(grid.shape[1]):
            ...
            neighbors = bin_get_neighbors(cell)
...
def bin_get_neighbors(cell: Cell):
    neighbors = []
...
def wilson(grid):
    ...
    while len(unvisited) > 0:
        ...
        # calculate grid and show to debug and see it work
